...
const CalciteAction = class {
    ...
    render() {
        ...
        return (index.h(index.Host, null, index.h("button", { class: index$1.classnames(CSS.button, buttonClasses), "aria-label": ariaLabel, disabled: disabled, "aria-disabled": disabled.toString(), "aria-busy": loading.toString(), ref: (buttonEl) => (this.buttonEl = buttonEl) }, this.renderIconContainer(), this.renderTextContainer())));
    }
...
const CalciteExpandToggle = ({ expanded, intlExpand, intlCollapse, toggleExpand, el, position, tooltipExpand }) => {
    ...
    const actionNode = (index.h("calcite-action", { ref: (referenceElement) => setTooltipReference(tooltipExpand, referenceElement, expanded), onClick: toggleExpand, textEnabled: expanded, text: expandText, icon: expanded ? expandIcon : collapseIcon }));
...
const CalciteActionBar = class {
    ...
    renderBottomActionGroup() {
        ...
        const expandToggleNode = expand ? (index.h(CalciteExpandToggle, { expanded: expanded, intlExpand: expandLabel, intlCollapse: collapseLabel, el: el, position: dom.getCalcitePosition(position, layout), toggleExpand: toggleExpand, tooltipExpand: tooltipExpand })) : null;
