...
import { layerList } from "../../icons/icons";
//----------------------------------
//
...
class ShareViewModel extends (Accessor) {
  ...
  @property() config: ApplicationConfig = null;
  ...
  private _generateShareUrlParams(point: Point): string {
    ...
    const { layerListPanel } = this.config;
    ...
    let shareParams = `${path}${sep}center=${roundedLon},${roundedLat}&level=${roundedZoom}${defaultObjectId}${attachmentIndex}${selectedLayerId}`;
    if (hiddenLayers?.length > 0 && layerListPanel) {
      shareParams += `&hiddenLayers=${hiddenLayers.join(",")}`
    }
